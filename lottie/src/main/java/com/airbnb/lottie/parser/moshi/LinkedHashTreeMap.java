 ...
 *
 * <p>This implementation was derived from TreeMap and
...
final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
   ...
   *     mutually comparable.
   * @return the node at or adjacent to the given key
   */
   ...
   * @return this map's entry that has the same key and value as {@code
   * entry}, or null if this map has no such entry.
   */
   ...
   * @param h hash code
   * @return hash function to a given hashCode
   */
  ...
  @Override public Set<K> keySet() {
    KeySet result = key_Set;
    return result != null ? result : (key_Set = new KeySet());
  }
   ...
   * @param oldTable array
   * @return Returns a new array containing the same nodes as
   */
