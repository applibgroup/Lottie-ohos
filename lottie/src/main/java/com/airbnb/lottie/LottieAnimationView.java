...
public class LottieAnimationView extends Image implements Component.BindStateChangedListener,
   ...
   /* @Override public void unscheduleDrawable(Drawable who) {
    ...
    /**
     * Sets a composition.
     * You can set a default cache strategy if this view was inflated with xml
     *
...
//        requestLayout();
    ...
    *//**
    ...
    private int lottieHeight(int sHeight){
        int LottieAnimationViewHeight = 0;
        if (getHeight() > 0) {
            LottieAnimationViewHeight = getHeight();
            if (LottieAnimationViewHeight > sHeight) {
                LottieAnimationViewHeight = sHeight;
            }
        } else {
            if (getComponentParent() instanceof ComponentContainer) {
                ComponentContainer container = (ComponentContainer) getComponentParent();
                LottieAnimationViewHeight = container.getHeight();
            }
            if (LottieAnimationViewHeight <= 0) {
                LottieAnimationViewHeight = sHeight;
            }
        }
        return LottieAnimationViewHeight;
    }

    private int lottieWidth(int sWidth)
    {
        int lottieAnimationViewWidth = 0;
        if (getWidth() > 0) {
            lottieAnimationViewWidth = getWidth();
            if (lottieAnimationViewWidth > sWidth) {
                lottieAnimationViewWidth = sWidth;
            }
        } else {
            if (getComponentParent() instanceof ComponentContainer) {
                ComponentContainer container = (ComponentContainer) getComponentParent();
                lottieAnimationViewWidth = container.getWidth();
            }
            if (lottieAnimationViewWidth <= 0) {
                lottieAnimationViewWidth = sWidth;
            }
        }
        return lottieAnimationViewWidth;
    }

    private ArrayList lottieAnimationSize(int sWidth, int sHeight)
    {
        // Calculate lottieAnimationView width and height values

        ArrayList lottieSize = new ArrayList();
        int lottieAnimationViewWidth = lottieWidth(sWidth);
        int LottieAnimationViewHeight = lottieHeight(sHeight);

        lottieSize.add(lottieAnimationViewWidth);
        lottieSize.add(LottieAnimationViewHeight);
    return lottieSize;
    }
    @Override
    public void onDraw(Component component, Canvas canvas) {
        if (composition != null) {
            ...
            int screenWidth = display.get().getAttributes().width;
            int screenHeight = display.get().getAttributes().height - 168;

            ArrayList lottieAnimationView = lottieAnimationSize(screenWidth,screenHeight);

            // Override setting lottieAnimationView width and height
            int lottieAnimationViewWidth = (int) lottieAnimationView.get(0);
            int LottieAnimationViewHeight = (int) lottieAnimationView.get(1);
            setWidth(lottieAnimationViewWidth);
            setHeight(LottieAnimationViewHeight);

            // Calculate the width and height of the canvas, to ensure width and height of the canvas are consistent with lottieAnimationView.
            float maxScaleX = (float) lottieAnimationViewWidth / composition.getBounds().getWidth();
            float maxScaleY = (float) LottieAnimationViewHeight / composition.getBounds().getHeight();
            float scale = Math.min(maxScaleX, maxScaleY);
            canvas.scale(scale, scale);

            // Move animation to center
            if (getLayoutConfig().width == ComponentContainer.LayoutConfig.MATCH_PARENT) {
                float width = (getWidth() / scale - composition.getBounds().getWidth()) / 2;
                canvas.translate(width, 0);
            } else if (getLayoutConfig().width != ComponentContainer.LayoutConfig.MATCH_CONTENT) {
                float width = ((float) getLayoutConfig().width / scale - (float) composition.getBounds().getWidth()) / 2;
                canvas.translate(width, 0);
            }
            if (getLayoutConfig().height == ComponentContainer.LayoutConfig.MATCH_PARENT) {
                float height = (getHeight() / scale - composition.getBounds().getHeight()) / 2;
                canvas.translate(0, height);
            } else if (getLayoutConfig().height != ComponentContainer.LayoutConfig.MATCH_CONTENT) {
                float height = ((float) getLayoutConfig().height / scale - (float) composition.getBounds().getHeight()) / 2;
                canvas.translate(0, height);
            }
        }
